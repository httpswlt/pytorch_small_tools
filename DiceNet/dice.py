...
class DICE(nn.Module):
    """
        This class implements the volume-wise separable convolutions
    """
    def __init__(self, in_channel, out_channel, height, width, ksize=3, dilation=[1, 1, 1], shuffle=True):
        """
            :param in_channel: # of input channels
            :param out_channel: # of output channels
            :param height: Height of the input volume
            :param width: Width of the input volume
            :param ksize: Kernel size. We use the same kernel size of 3 for each dimension. Larger kernel size would increase the FLOPs and Parameters
        ...
        assert in_channel >= 4, "Please Check Input Channel, It should >= 4."
        padding_1 = int((ksize - 1) / 2) * dilation[0]
        padding_2 = int((ksize - 1) / 2) * dilation[1]
        padding_3 = int((ksize - 1) / 2) * dilation[2]
        self.conv_channel = nn.Conv2d(in_channel, in_channel, kernel_size=ksize, stride=1, groups=in_channel,
                                      padding=padding_1, bias=False, dilation=dilation[0])
        self.conv_width = nn.Conv2d(width, width, kernel_size=ksize, stride=1, groups=width, padding=padding_2,
                                    bias=False, dilation=dilation[1])
        self.conv_height = nn.Conv2d(height, height, kernel_size=ksize, stride=1, groups=height, padding=padding_3,
                                     bias=False, dilation=dilation[2])
        self.br_act = BR(3 * in_channel)
        ...
        self.linear_comb_layer = nn.Sequential(
            ...
            nn.Conv2d(in_channel, in_channel // 4, kernel_size=1, bias=False),
...
class StridedDICE(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, height, width, ksize=3, dilation=[1, 1, 1], shuffle=True):
        ...
        self.left_layer = nn.Sequential(
            CBR(in_channel, in_channel, ksize=3, stride=2, groups=in_channel),
            CBR(in_channel, in_channel, ksize=1, stride=1)
        )

        self.right_layer = nn.Sequential(
            nn.AvgPool2d(kernel_size=3, padding=1, stride=1),
            DICE(in_channel, in_channel, height, width, ksize=ksize, dilation=dilation, shuffle=shuffle),
            CBR(in_channel, in_channel, ksize=1, stride=1)
        )
