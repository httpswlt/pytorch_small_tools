...
def test(
    ...
    for batch_i, (imgs, targets) in enumerate(tqdm(dataloader, desc='Computing mAP')):
        ...
        # Run model
        inf_out, loss_i, loss_item = model(imgs, targets)  # inference and training outputs
        ...
        true_targets = targets[torch.sum(targets[:, 1:6], 1) != 0] # remove the targets that fills 0 for data distribution.
