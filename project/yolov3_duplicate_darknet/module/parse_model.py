...
from module.modules import EmptyLayer, MaxPoolStride1, YOLOLayer, Upsample
import pdb
...
def create_modules(blocks):
    ...
    for index, x in enumerate(blocks[1:]):
        ...
        if x["type"] == "convolutional":
            ...
            ignore_thres_fl = float(x["ignore_thresh"]) # ignore threshold for first loss
            mask = x["mask"].split(",")
            mask = [int(i) for i in mask]
            anchors = x["anchors"].split(",")
            anchors = [float(a) for a in anchors]
            # Anchors of all yolo layers
            ANCHORS = [(anchors[i], anchors[i + 1]) for i in range(0, len(anchors), 2)]
            anchors = [ANCHORS[i] for i in mask]
            detection = YOLOLayer(anchors, ANCHORS, mask, int(x['classes']),
                                  (net_info['width'], net_info['height']), ignore_thres_fl)
